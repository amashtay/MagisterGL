...
class ShaderProgram
{
  ...
  func attachShader(_ file: String,
                    withType type: GLint)
  {
    if let shader = compileShader(file,
  ...
  {
    let tmp = glGetAttribLocation(program,
                                  name.cString(using: String.Encoding.utf8)!)
    return tmp < 0 ? nil : GLuint(tmp)
  }
  
  func getUniformLocation(_ name: String) -> GLuint?
  {
    let tmp = glGetUniformLocation(program,
                                   name.cString(using: String.Encoding.utf8)!)
    return tmp < 0 ? nil : GLuint(tmp)
  }
  ...
  fileprivate func getGLShaderInfoLog(_ shader: GLuint) -> String
  {
    ...
    glGetShaderiv(shader,
                  GLenum(GL_INFO_LOG_LENGTH), &length)
    
    // Retrieve the info log.
    var str = [GLchar](repeating: GLchar(0),
                       count: Int(length) + 1)
    var size: GLsizei = 0
  ...
  fileprivate func compileShader(_ file: String,
                                 withType type: GLenum) -> GLuint?
  {
