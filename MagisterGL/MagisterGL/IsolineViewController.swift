      ...
      for y in yArray
      {
        let element = RectangleElement(startPoint: CGPoint(x: x.doubleValue,
                                       ...
                                       size: CGSize(width: hX,
                                                    height: hY))
        element.topColor1 = getColorForU(u: uFunc(x: Double(x), y: Double(y)))
        element.topColor2 = getColorForU(u: uFunc(x: Double(x) + hX, y: Double(y)))
        element.bottomColor1 = getColorForU(u: uFunc(x: Double(x), y: Double(y) + hY))
        element.bottomColor2 = getColorForU(u: uFunc(x: Double(x) + hX, y: Double(y) + hY))
